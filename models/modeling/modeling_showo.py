...
class Showo(ModelMixin, ConfigMixin):
        ...
        **kwargs,
        ...
        if labels is not None:
            ...
            loss_t2i = F.cross_entropy(
                logits[:batch_size_t2i, max_seq_length + 1 :]
                .contiguous()
            ...
            loss_lm = F.cross_entropy(
                logits[batch_size_t2i : batch_size_t2i + batch_size_lm, :-1]
                .contiguous()
                .view(-1, self.output_size),
                labels[batch_size_t2i : batch_size_t2i + batch_size_lm, 1:]
                .contiguous()
                .view(-1),
                ignore_index=-100,
            )
            ...
            if batch_size_mmu > 0:
                ...
                loss_mmu = torch.tensor(0.0, device=logits.device, requires_grad=True)
    ...
    def t2i_generate(
        ...
        **kwargs,
        ...
        if uncond_input_ids is not None:
            uncond_prefix = uncond_input_ids[
                :, : config.dataset.preprocessing.max_seq_length + 1
            ]
        ...
        for step in range(timesteps):
            if uncond_input_ids is not None and guidance_scale > 0:
                ...
                logits = (
                    1 + guidance_scale
                ) * cond_logits - guidance_scale * uncond_logits
                logits = logits[
                    :,
                    -(num_vq_tokens + 1) : -1,
            ...
            sampled_ids = torch.where(
                unknown_map, sampled_ids, input_ids_minus_lm_vocab_size
            )
            ...
            selected_probs = torch.where(
                unknown_map, selected_probs, torch.finfo(selected_probs.dtype).max
            )
            # Gets mask lens for each sample in the batch according to the mask ratio.
            mask_len = (
                (num_vq_tokens * mask_ratio).floor().unsqueeze(0).to(logits.device)
            )
            ...
            masking = mask_by_random_topk(
                mask_len, selected_probs, temperature, generator=generator
            )
            ...
            input_ids_minus_lm_vocab_size = torch.where(
                masking, mask_token_id, sampled_ids
            )
    ...
    def mmu_generate(
        ...
        for _ in range(max_new_tokens):
            ...
            logits = self(
                idx, input_embeddings=input_embeddings, attention_mask=attention_mask
            )
            ...
            if self.config.w_clip_vit:
                ...
                input_embeddings = torch.cat(
                    [input_embeddings, idx_next_embeddings], dim=1
                )
            else:
